...
local function cleanupClone()
    safeDisconnect(invisDiedConn); invisDiedConn = nil
    safeDisconnect(fallCheckConn); fallCheckConn = nil
...
local function setCameraToCharacterChar(char)
    pcall(function()
...
local function transferTools(fromChar, toChar)
...
local function applyInvisibilityToCharacter(char)
    cleanupClone()
    ...
    -- transferir herramientas antes de mover OriginalCharacter fuera
    transferTools(OriginalCharacter, InvisibleCharacter)

    -- mantener posición y swap
    local cf = OriginalCharacter.HumanoidRootPart.CFrame
    -- mover original a Lighting para "archivarlo" sin romper el cliente
    OriginalCharacter.Parent = Lighting
    InvisibleCharacter.Parent = workspace
    InvisibleCharacter.HumanoidRootPart.CFrame = cf

    -- asignar character y cámara
    player.Character = InvisibleCharacter
    setCameraToCharacterChar(InvisibleCharacter)

    IsInvis = true

    -- si el clon muere, limpiamos y dejamos que CharacterAdded reaplique invis si corresponde
    local hum = InvisibleCharacter:FindFirstChildOfClass("Humanoid")
    if hum then
        invisDiedConn = hum.Died:Connect(function()
            cleanupClone()
            -- no forzamos nada aquí; CharacterAdded se encargará del nuevo character
        end)
    end

    -- chequeo de caída fuera del mapa
    fallCheckConn = RunService.Stepped:Connect(function()
        pcall(function()
            if not player.Character or not player.Character:FindFirstChild("HumanoidRootPart") then return end
            local y = player.Character.HumanoidRootPart.Position.Y
            local void = workspace.FallenPartsDestroyHeight
            local isNeg = tostring(void):find('-') and true or false
...
local function TurnVisible()
    ...
    cleanupClone()
    -- si el OriginalCharacter existe, restaurarlo al workspace y reasignar cámara
    if OriginalCharacter then
        -- intentar mantener la posición del humano actual
        pcall(function()
            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and OriginalCharacter:FindFirstChild("HumanoidRootPart") then
                OriginalCharacter.HumanoidRootPart.CFrame = player.Character.HumanoidRootPart.CFrame
            end
        end)
        OriginalCharacter.Parent = workspace
        player.Character = OriginalCharacter
        setCameraToCharacterChar(OriginalCharacter)
    end
    IsInvis = false
end

-- manejar respawn / morph
charAddedConn = player.CharacterAdded:Connect(function(newChar)
    -- pequeña espera para que el character esté listo
    task.wait(0.05)
    if IsInvis then
        -- reaplicar invis al nuevo character (respawn o morph)
        applyInvisibilityToCharacter(newChar)
    else
        -- actualizar referencia del original para futuros toggles
        OriginalCharacter = newChar
        -- asegurar cámara
        setCameraToCharacterChar(newChar)
    end
end)

-- Toggle para el menú
createToggle("Invisible", function(getState)
    if getState then
        if IsInvis then return end
        -- esperar a que el character esté listo
        repeat task.wait(0.05) until player.Character and player.Character:FindFirstChild("HumanoidRootPart")
        applyInvisibilityToCharacter(player.Character)
    else
        TurnVisible()
    end
end)
